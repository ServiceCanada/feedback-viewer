...
/**
 * Extension of MongoRepository that adds DataTables server-side processing support. Provides
 * findAll methods that accept DataTablesInput and return DataTablesOutput. Compatible with Azure
...
public interface DataTablesRepository<T, ID> extends MongoRepository<T, ID> {
  ...
  /**
   * Find all entities matching the DataTables request parameters with additional MongoDB criteria.
   */
  DataTablesOutput<T> findAll(DataTablesInput input, Criteria additionalCriteria);

  /**
   * Find all entities matching the DataTables request parameters with additional MongoDB criteria
   * and a pre-computed total count. This avoids expensive count queries on large collections.
   *
